export interface Env {
  ...
  WATCHDOG_HMAC_SECRET?: string;
  WATCHDOG_DASHBOARD_TOKEN?: string;
...
async function verifyIngestRequest(request: Request, env: Env, rawBody: string): Promise<boolean> {
...
function validateIncidentEnvelope(value: unknown): IncidentEnvelope {
  if (!value || typeof value !== "object") {
    throw new Error("Payload must be an object.");
  }
  const envelope = value as IncidentEnvelope;
...
async function storeIncident(env: Env, envelope: IncidentEnvelope): Promise<void> {
  ...
  await env.DB.prepare(
    ...
    .bind(
      ...
      JSON.stringify(incident.repeated_files),
      JSON.stringify(incident.repeated_errors),
      JSON.stringify(incident.triggering_event_ids),
...
async function sendSlackAlert(env: Env, envelope: IncidentEnvelope): Promise<void> {
...
async function sendEmailAlert(env: Env, envelope: IncidentEnvelope): Promise<void> {
...
async function listIncidents(request: Request, env: Env): Promise<Response> {
  const auth = request.headers.get("authorization");
  ...
  const result = await env.DB.prepare(
    "SELECT incident_id, session_id, created_at, score, reasons_json, repeated_files_json, repeated_errors_json, request_count, recommendation FROM incidents ORDER BY created_at DESC LIMIT ?",
  )
    .bind(limit)
