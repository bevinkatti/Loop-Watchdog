...
def test_session_identity_creation() -> None:
    identity = SessionIdentity(
        watchdog_session_id="unique-123", repository="my-repo", branch="main", agent="codex"
    )
...
def test_legacy_session_id_mapping() -> None:
    ...
    event = WatchdogEventCreate(session_id="legacy:session:1", kind=EventKind.FILE_EDIT)
